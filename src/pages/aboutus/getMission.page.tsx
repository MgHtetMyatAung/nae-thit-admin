...
export default function GetMissionPage(){
    ...
    return (
        <div>
            <div className="flex justify-between">
               <div>
               <h1 className="text-3xl text-gray-700">Exisitng missions on the page!</h1>
               <p className="text-gray-800">You can either edit or delete the mission cards!</p>
               </div>

               <Link to="/aboutmission/create">
                <button className="bg-blue-500 py-3 px-4 text-white text-xl">Create</button>
               </Link>
            </div>
  ...
  {MissionsData?.data?.map((mission: any, index: number) => (
    ...
    >
      ...
      <button onClick={()=>navigate(`/aboutmission/edit/${mission?._id}`)} className="text-blue-800">
        <SquarePen/>
      </button>
